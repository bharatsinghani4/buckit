...
export function api<T>(path: string, options: ApiOptions = {}): Promise<Envelope<T>> {
  ...
  if (!user) return Promise.reject(new ClientError("Sign in to continue.", 401, "AUTHENTICATION_REQUIRED"));
  const readKey = (!options.method || options.method === "GET") && options.body === undefined ? `${user.uid}:${path}` : null;
  if (readKey) {
  ...
  if (readKey) {
    ...
    const clear = () => { if (pendingReads.get(readKey) === request) pendingReads.delete(readKey); };
...
async function sendApi<T>(user: User, path: string, options: ApiOptions): Promise<Envelope<T>> {
  ...
  if (response.status === 401) {
    try { headers.Authorization = `Bearer ${await user.getIdToken(true)}`; response = await send(); }
    catch { throw new ClientError("Your session could not be refreshed. Check your connection and sign in again.", 401, "SESSION_REFRESH_FAILED"); }
  }
  let result: { data?: T; meta?: Envelope<T>["meta"]; error?: { message?: string; code?: string; fieldErrors?: { path: string; message: string }[] } };
  try { result = await response.json(); }
  catch { throw new ClientError("Buckit returned an unexpected response. Try again shortly.", response.status, "INVALID_RESPONSE"); }
