...
export function assertRevision(header: string | null, revision: number) {
  ...
  if (header !== `"r${revision}"`)
    throw new ApiError(
      412,
      "REVISION_MISMATCH",
      "This changed in another session. Refresh and try again.",
    );
}
...
export function requireIdempotencyKey(request: Request) {
  ...
  if (!key || !/^[a-zA-Z0-9_-]{16,128}$/.test(key))
    throw new ApiError(400, "MALFORMED_REQUEST", "A valid operation key is required.");
  return key;
}
...
export async function readJson(request: Request): Promise<unknown> {
  ...
  try {
    ...
    throw new ApiError(400, "MALFORMED_REQUEST", "The request is not valid JSON.");
  }
}
