...
function CreateBucketForm({ additional }: { additional: boolean }) {
  ...
  return <main id="main" className="onboarding-page"><div className="onboarding-top"><Brand />{profile && <Link href="/workspace" className="text-link">Your workspace</Link>}</div><div className="onboarding-content"><header className="onboarding-heading"><span className="eyebrow">{additional ? "A NEW SPACE" : "INITIAL SETUP · STEP 1 OF 1"}</span><h1>{additional ? "Make room for a new bucket." : <>Welcome to <Wordmark /></>}</h1><p>Let’s set up your profile and create a space<br className="desktop-break" /> for the spending that matters to you.</p></header>
    <form className="onboarding-card form-stack" onSubmit={submit}>{error && <Notice>{error}</Notice>}<section><div className="section-label"><span className="step-number">1</span><h2>Your identity</h2><span className="pill">Your profile</span></div><div className="form-grid"><label>Display name<input name="displayName" placeholder="Your name" defaultValue={user?.displayName || profile?.displayName || ""} required maxLength={80} autoComplete="name" /></label><label>Your timezone<select value={timezone} onChange={(e) => setTimezone(e.target.value)}>{zones.map((zone) => <option key={zone}>{zone}</option>)}</select></label></div></section>
    <div className="rule" /><section><div className="section-label"><span className="step-number filled">2</span><h2>{additional ? "Your new bucket" : "Your first bucket"}</h2></div><p className="section-description">A bucket keeps spending for one part of your life together.</p><div className="suggestions"><span>SUGGESTIONS</span><button type="button" aria-pressed={name === "House Expenses"} className={name === "House Expenses" ? "selected" : ""} onClick={() => setName("House Expenses")}><Home size={14} /> House Expenses</button><button type="button" aria-pressed={name === "Personal"} className={name === "Personal" ? "selected" : ""} onClick={() => setName("Personal")}><UserRound size={14} /> Personal</button></div><div className="form-stack"><label>Bucket name<input value={name} onChange={(e) => setName(e.target.value)} required maxLength={80} placeholder="e.g. House Expenses, Personal, Travel" /></label><label><span className="label-row">Primary currency<small>Fixed after first expense</small></span><select name="primaryCurrency" defaultValue="INR">{currencies.map((currency) => <option key={currency.code} value={currency.code}>{currency.code} ({currency.symbol}) — {currency.name}</option>)}</select></label><div className="timezone-note"><span><Clock3 size={16} /> Bucket timezone</span><b>{timezone}</b></div><p className="field-hint"><Info size={14} /> Currency is fixed after your first expense.</p></div></section>
