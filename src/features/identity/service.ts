...
export async function profileDto(
  user: AppUser,
  identity: DecodedIdToken,
  session?: ClientSession,
): Promise<Profile> {
  ...
  return {
    ...
    tour: {
      version: 1,
      state: user.tour?.state ?? "not_started",
      lastStep: user.tour?.lastStep ?? 0,
    },
    revision: user.revision,
  };
}

export async function bucketForUser(
  id: string,
  user: AppUser,
  session?: ClientSession,
  write = false,
  ...
  if (write) {
    if (bucket.status !== "active")
      throw new ApiError(409, "BUCKET_ARCHIVED", "This bucket is archived.");
...
export async function bucketDto(
  id: string,
  user: AppUser,
  session?: ClientSession,
): Promise<Bucket> {
  ...
  return {
    ...
    status: b.status,
    revision: b.revision,
    isOwner: String(b.ownerUserId) === String(user._id),
    memberCount,
  };
}
...
export async function mutate(
  ...
  key: string,
  input: unknown,
  perform: (session: ClientSession) => Promise<MutationResult>,
  replay: (id: string, session: ClientSession) => Promise<unknown>,
) {
  const digest = requestHash(input);
  return mongoose.connection.transaction(async (session) => {
    const receipt = await ReceiptModel.findOne({ actor: identity.uid, scope, key }).session(
      session,
    );
    if (receipt) {
      ...
      return {
        data: await replay(receipt.resourceId, session),
  ...
  return mutate(
    ...
    async (session) => {
          ...
          [
            {
              ...
              displayName: input.displayName ?? identity.name?.slice(0, 80) ?? "New member",
      ...
      return {
        ...
        data: await profileDto(user, identity, session),
        location: "/api/v1/me",
      };
    },
...
export async function updateProfile(
  ...
  return mutate(
    ...
    async (session) => {
      ...
      assertRevision(etag, user.revision);
      if (input.lastBucketId) await bucketForUser(input.lastBucketId, user, session);
      const updated = await UserModel.findOneAndUpdate(
        { _id: user._id, revision: user.revision, status: "active" },
      ...
      return {
        resourceId: String(user._id),
        status: 200,
        data: await profileDto(updated, identity, session),
      };
    },
    async (_id, session) => profileDto(await guardUser(identity, session), identity, session),
  );
...
export async function createBucket(identity: DecodedIdToken, raw: unknown, key: string) {
  ...
  return mutate(
    ...
    async (session) => {
      ...
      await UserModel.updateOne(
        { _id: user._id },
        { $set: { lastBucketId: bucket._id }, $inc: { revision: 1 } },
        { session },
      );
      ...
      return {
        ...
        status: 201,
        data: await bucketDto(String(bucket._id), user, session),
        location: `/api/v1/buckets/${bucket._id}`,
      };
    },
  ...
  return mutate(
    ...
    async (session) => {
      ...
      return {
        ...
        location: `/api/v1/buckets/${bucketId}/invitations`,
      };
    },
    async (id, session) => {
      const user = await guardUser(identity, session);
      await bucketForUser(bucketId, user, session, false, true);
      const invite = await InvitationModel.findById(id).session(session);
...
async function validInvitation(token: string, session?: ClientSession) {
  ...
  const bucket = await BucketModel.findOne({ _id: invite.bucketId, status: "active" }).session(
    session ?? null,
  );
...
export async function previewInvitation(identity: DecodedIdToken, raw: unknown) {
  const { token } = invitationSchema.parse(raw);
...
export async function joinInvitation(identity: DecodedIdToken, raw: unknown, key: string) {
  ...
  return mutate(
    ...
    async (session) => {
      const user = await guardUser(identity, session);
      const { invite, bucket } = await validInvitation(input.token, session);
      await BucketModel.updateOne(
        { _id: bucket._id, status: "active" },
        { $inc: { writeRevision: 1 } },
        { session },
      ...
      const guard = await InvitationModel.updateOne(
        { _id: invite._id, revokedAt: null, expiresAt: { $gt: new Date() } },
        { $inc: { revision: 1 } },
        { session },
      );
      if (!guard.matchedCount) throw unavailableInvite();
      const existing = await MembershipModel.exists({
        bucketId: bucket._id,
        userId: user._id,
        state: "active",
      }).session(session);
...
export async function listBuckets(identity: DecodedIdToken, cursor: string | null) {
  ...
  if (cursor) {
    try {
      ...
      if (
        parsed.user !== String(user._id) ||
        parsed.route !== "buckets" ||
        parsed.expires < Date.now() ||
        !/^[a-f\d]{24}$/i.test(parsed.after)
      )
        throw new Error();
      after = parsed.after;
    } catch {
      throw new ApiError(400, "INVALID_CURSOR", "Refresh the bucket list.");
    }
  }
  const memberships = await MembershipModel.aggregate([
    {
      $match: {
        userId: user._id,
        state: "active",
        ...(after ? { bucketId: { $gt: new mongoose.Types.ObjectId(after) } } : {}),
      },
    },
    { $lookup: { from: "buckets", localField: "bucketId", foreignField: "_id", as: "bucket" } },
    { $unwind: "$bucket" },
    { $match: { "bucket.status": { $in: ["active", "archived"] } } },
    { $sort: { bucketId: 1 } },
    { $limit: 26 },
  ]);
  const selected = memberships.slice(0, 25);
  const data: Bucket[] = [];
  ...
  if (memberships.length > 25) {
    const payload = Buffer.from(
      JSON.stringify({
        ...
        route: "buckets",
        after: String(selected.at(-1).bucketId),
        expires: Date.now() + 30 * 60_000,
      }),
    ).toString("base64url");
    nextCursor = `${payload}.${signCursor(payload)}`;
  }
