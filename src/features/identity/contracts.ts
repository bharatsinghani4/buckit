...
export const tourSchema = z
  .object({
    ...
    lastStep: z.number().int().min(0).max(3),
  })
  .strict();
...
export const bucketSchema = z
  .object({
    name: z.string().trim().min(1, "Name your bucket.").max(80),
    primaryCurrency: z
      .string()
      .refine((code) => currencies.some((c) => c.code === code), "Choose a supported currency."),
    timezone: timezoneSchema,
  })
  .strict();
export const invitationSchema = z
  .object({ token: z.string().regex(/^[A-Za-z0-9_-]{43}$/, "This invitation link is not valid.") })
  .strict();
